 ...
 * @author fxf
 * 
 *         极大团挖掘的TTT算法，由SparkMCEx调用，计算极大团
 */
public class SparkTTT {
	public ArrayList<ArrayList<String>> graph; // 图的全部信息
	public ArrayList<String> nodes; // 所求极大团的节点的集合
	public ArrayList<ArrayList<String>> cliques;// 求得的全部团

	public SparkTTT(ArrayList<ArrayList<String>> g, ArrayList<String> n) {
	...
	private void TTTAlgth(ArrayList<String> SUBG, ArrayList<String> CAND, ArrayList<String> Q) {
		...
		if (SUBG.isEmpty()) {// 求出极大团的条件，SUBG为空
			if (!(Q.isEmpty())) {
				...
				cliques.add(clique);
				// System.out.println("cliques:" + clique.toString());
			}

		} else {

			String v = SUBG.get(0);// SUBG中的点v，其邻点与CAND集合相交所得集合中节点数最多
			ArrayList<String> vNeibors = findNeibors(v); // 获得顶点v的全部邻点
			...
			for (String q : qs) {
				ArrayList<String> qNeibors = findNeibors(q);
	 ...
	 * 		寻找点v的邻点集合
	 */
	private ArrayList<String> findNeibors(String v) {
		// TODO Auto-generated method stub
		ArrayList<String> neibors = new ArrayList<>();
		for (ArrayList<String> n : graph) {
			// System.out.println(graph.toString());
			if (n.get(0).equals(v)) {
				neibors = (ArrayList<String>) n.clone();
				break;

			}
		}
		// System.out.println("neibors:" + neibors.toString());
		if (!(neibors.isEmpty())) {
			neibors.remove(0);
		}
		return neibors;
	...
	private String findVS(ArrayList<String> sUBG, ArrayList<String> cAND) {
		...
		for (String n : sUBG) {
			...
			ArrayList<String> vNeibors = findNeibors(n);

...
public class SparkTTT_V1_3 {
	...
	private void TTTAlgth(ArrayList<String> SUBG, ArrayList<String> CAND, ArrayList<String> Q) {
		...
		if (SUBG.isEmpty()) {// 求出极大团的条件，SUBG为空
			...
			String v = findVS(SUBG, CAND);// SUBG中的点v，其邻点与CAND集合相交所得集合中节点数最多
			...
			for (int i = 0; i < CAND.size(); i++) {
				if (Integer.parseInt(v) > Integer.parseInt(CAND.get(i))) {// 通过顶点偏序策略来解决计算冗余问题
					// System.out.println("v:" + Integer.parseInt(v) + " CAND.get(i):" +
					// Integer.parseInt(CAND.get(i)));
					CAND.remove(i);
					i--;
				}
			}

			// System.out.println("CAND_mid:" + CAND.toString());
